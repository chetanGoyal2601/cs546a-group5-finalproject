$(document).ready(function () {
    ...
    success: function (result, status, xhr) {
      if (result.length == 0) {
        ...
        const locations = {
          ...
          cities: Array.from(
            new Set(allUniversities.map((uni) => uni.location.city))
          ),
        };
        ...
        header.appendChild(
          document
            .createElement("h1")
            .appendChild(document.createTextNode("STATES"))
        );
  ...
  $("#pagination").click(function (event) {
    ...
    if (pageNo === "Previous") {
      if (+current.id != 1) {
        ...
        populateArray(
          uniArray.slice(
            PAGE_SIZE * (+current.id - 2),
            PAGE_SIZE * (+current.id - 1)
          )
        );
      }
    } else if (pageNo === "Next") {
      if (+current.id != Math.ceil(uniArray.length / PAGE_SIZE)) {
        ...
        document.getElementById(+current.id + 1).classList.add("active");
        populateArray(
          uniArray.slice(PAGE_SIZE * +current.id, PAGE_SIZE * (+current.id + 1))
        );
      }
    } else {
      current.classList.remove("active");
      ...
      populateArray(
        uniArray.slice(PAGE_SIZE * (pageNo - 1), PAGE_SIZE * pageNo)
      );
    }
  });
  ...
  function populateArray(paginationArray) {
    ...
    for (let i = 0; i < paginationArray.length; i++) {
      ...
      card.setAttribute(
        "href",
        "/university/" + paginationArray[i]._id.toString()
      );
      ...
      rank.id = "rank1";
      intro.appendChild(
        document.createTextNode(paginationArray[i].introduction)
      );
      rank.appendChild(
        document.createTextNode("Rank: " + paginationArray[i].ranking)
      );
      ul.appendChild(rank);
  ...
  $("#locList").on("change", function () {
    if ($(this).val() === "") {
      ...
      $("#uniList").empty();
      uniArray = allUniversities.filter(
        (uni) =>
          uni.location.state === $("#locList").val() ||
          uni.location.city === $("#locList").val()
      );
      paginationBar();
    }
  });
      ...
      uniArray = $("#locList").val()
        ? allUniversities.filter(
            (uni) =>
              ...
              uni.name
                .toLowerCase()
                .includes($("#uniSearchTerm").val().toLowerCase())
          )

...
const validation = require("./validationDev");
...
let exportedMethods = {
  ...
  async postCommentOnUniversity(comment, universityId) {
    ...
    if (insertInfo.insertCount === 0 || !insertInfo.insertedId)
      throw `could not add comments`;
    let newId = insertInfo.insertedId.toString();
    if (newId) {
      const insertComments = await this.addCommentsToUniversity(
        universityId,
        newId
      );
    }
  ...
  async addCommentsToUniversity(id, commentsId) {
    ...
    if (!ObjectId.isValid(parsedId)) throw "Object Id is not valid";
    const universityCollection = await universitiesMongo();
    const data = await universityCollection.updateOne(
      { _id: parsedId },
      {
        $push: {
  ...
  async getCommentsByIds(inputCommentIds) {
    ...
    let commentList = await commentsCollection
      ...
      .toArray();
    if (!commentList)
      throw { message: "Could not get all comments", code: 400 };
  ...
  async addFavUniversity(id, userID) {
    ...
    if (!ObjectId.isValid(parsedId)) throw "Object Id is not valid";
    const profileCollection = await profileMongo();
    const data = await profileCollection.updateOne(
      { _id: ObjectId(userID) },
      {
        $push: {

  ...
  try {
    ...
    for (const i in postDataList) {
      output.push({
        ...
        userId: "6275df675b007316142f0b8a",
...
router.route("/posts/like").post(async (req, res) => {
  ...
  //let userId = req.body.userId;
  let userId = "6275df675b007316142f0b8a";
...
router.route("/post").post(async (req, res) => {
  ...
  try {
    idValidation("6275df675b007316142f0b8a"); // ObjectIdValidation and if the user exists in db or not
  ...
  try {
    const p = await postData.createPost("6275df675b007316142f0b8a", postInfo);

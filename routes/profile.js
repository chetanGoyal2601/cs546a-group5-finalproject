...
var checkpassword = async function (email, password){
    ...
    for (var i = 0; i < getuser.length; i++) {
        if ((getuser[i].email == email) && bcrypt.compareSync(password, getuser[i].password)) {
            return true;
        }
    }
    return false;
...
var checkvalid = async function (req) {
    ...
    if (!email.match(/^[a-z0-9_@\.]+$/)) throw "Enter a User Name only with valid characters"
...
router.get('/signup', function (req, res, next) {
    if (checkLog(req)) {
        ...
        res.render('profile/signup', { title: "SignUp",  
        isUserLoggedIn: req.session.user != null ? true : false })
    }
})
...
router.post('/signup', async function (req, res) {
    ...
    // }
        ...
        errorMessage = "Enter a password with more than 8 and less than 20 characters";
    }
    ...
    res.render('profile/signup', { title: "Error", error: errorMessage, isUserLoggedIn: req.session.user != null ? true : false })
});
...
router.get('/profile', async function (req, res) {
    ...
    if (req.session.user) {
        ...
        res.render('profile/userProfile',
         { title: "Information", uniArr: uniArr,
         user: getuser, isUserLoggedIn: req.session.user != null ? true : false })
    }
    else {
        res.redirect('/');
    }
...
router.get('/update_profile', async function (req, res) {
    if (req.session.user) {
        const getuser = await profileFetch.getUserById(req.session.user);
        res.render('profile/updateProfile', { title: "Update Profile", user: getuser, isUserLoggedIn: req.session.user != null ? true : false })
    }
...
//     }
    ...
    try {
        if (errorMessage == null) {
            ...
            const updateuser = await profileFetch.updateUser(req.session.user, updatedUser);
    ...
    const getuser = await profileFetch.getUserById(req.session.user);
    //console.log(errorMessage);
    res.render('profile/updateProfile', { title: "Error", error: errorMessage, user: getuser, isUserLoggedIn: req.session.user != null ? true : false })
...
router.get('/update_password', async function (req, res) {
    if (req.session.user) {
        const getuser = await profileFetch.getUserById(req.session.user);
        res.render('profile/change_password', { title: "Update Profile", user: getuser, isUserLoggedIn: req.session.user != null ? true : false })
    }
...
router.post('/update_password', async function (req, res) {
    ...
    const getuser = await profileFetch.getUserById(req.session.user);
    //console.log(errorMessage);
    res.render('profile/change_password', { title: "Error", error: errorMessage, user: getuser, isUserLoggedIn: req.session.user != null ? true : false })
